...
export default function CheckoutPage() {
    ...
    }, [user, token, router, showToast, loading]);
    ...
    const handleCheckout = async () => {
        ...
        if (itemsWithoutSize.length > 0) {
            const names = itemsWithoutSize.map(i => i.name || `Sản phẩm #${i.product_id}`).join(", ");
        ...
        if (useNewAddress) {
            if (!newAddress.fullName?.trim()) return showToast("Vui lòng nhập họ tên", "error");
            if (!newAddress.phone?.trim()) return showToast("Vui lòng nhập số điện thoại", "error");
            if (!newAddress.addressLine?.trim()) return showToast("Vui lòng nhập địa chỉ", "error");
            ...
            try {
                ...
                const addrData = await addrRes.json().catch(() => ({}));
                if (!addrRes.ok) {
                    showToast(addrData.message || "Tạo địa chỉ thất bại", "error");
                    return;
                }
        ...
        return (
            <div className="checkout-container">
                ...
                <div className="checkout-loading">
                    <p>Bạn phải đăng nhập để thực hiện thanh toán.</p>
                    <div style={{ marginTop: 12 }}>
                        <button
                            className="actionBtn actionBtnPrimary"
                            onClick={() => router.push('/login?callback=/checkout')}
                        >
                            Đăng nhập
                        </button>
                        <button
                            ...
                            {useNewAddress && (
                                ...
                                >
                                    <label style={{ display: "block", marginBottom: 10 }}>
                                        <span style={{ fontWeight: 600, marginBottom: 4, display: "block" }}>Họ tên</span>
                                        <input
                                            type="text"
                                    ...
                                    <label style={{ display: "block", marginBottom: 10 }}>
                                        <span style={{ fontWeight: 600, marginBottom: 4, display: "block" }}>Số điện thoại</span>
                                        <input
                                            type="text"
                                    ...
                                    <label style={{ display: "block", marginBottom: 10 }}>
                                        <span style={{ fontWeight: 600, marginBottom: 4, display: "block" }}>Địa chỉ</span>
                                        <textarea
